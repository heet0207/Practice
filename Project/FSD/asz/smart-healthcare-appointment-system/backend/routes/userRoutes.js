...
// @desc   Get a single patient's profile (for doctors reviewing a booking)
router.get("/patients/:id", (req, res) => {
  const id = parseInt(req.params.id);
  const patient = db.users.find((u) => u.id === id && u.role === "patient");
  if (!patient) {
    return res.status(404).json({ message: "Patient not found" });
  }
  // Never send the password back, even though this is a demo app.
