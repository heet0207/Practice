...
function DoctorDashboard() {
  ...
  const handleMail = async (id) => {
    ...
    try {
      const res = await API.post(`/appointments/${id}/send-confirmation`);
      const nextStatus = res.data.emailConfigured === false ? "not_configured" : "sent";
      setMailStatus((s) => ({ ...s, [id]: nextStatus }));
      setTimeout(() => setMailStatus((s) => ({ ...s, [id]: undefined })), 4000);
    } catch (err) {
                    ...
                    <td className="row-actions">
                      ...
                      {appt.status === "Confirmed" && (
                        <button
                          ...
                          title={mailStatus[appt.id] === "not_configured" ? "The server has no email account configured yet — see backend/.env.example" : undefined}
                        >
                          <IconMail />
                          {mailStatus[appt.id] === "sending"
                            ...
                            ? "Sent!"
                            : mailStatus[appt.id] === "not_configured"
                            ? "Email not set up"
